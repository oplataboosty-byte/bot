...
# ── API SERVER (встроен в бота) ───────────────────────────────────────────────
API_SECRET = os.environ.get("API_SECRET", "Yrener2011")
...
async def handle_check_key(request):
    ...
    if secret != API_SECRET:
        return web.Response(
            text=json.dumps({"valid": False, "reason": "forbidden"}),
            content_type="application/json", status=403
        )

    if not key:
        return web.Response(
            text=json.dumps({"valid": False, "reason": "no_key"}),
            content_type="application/json"
        )

    # Ищем в активированных пользователях
    with get_conn() as conn:
        row = conn.execute("SELECT * FROM users WHERE key=?", (key,)).fetchone()

    if not row:
        return web.Response(
            text=json.dumps({"valid": False, "reason": "not_found"}),
            content_type="application/json"
        )
    ...
    if now > expires:
        return web.Response(
            text=json.dumps({"valid": False, "reason": "expired"}),
            content_type="application/json"
        )
...
async def start_web_server():
    ...
    app.router.add_get("/check_key", handle_check_key)
    runner = web.AppRunner(app)
    await runner.setup()
    site = web.TCPSite(runner, "0.0.0.0", WEB_PORT)
    await site.start()
    print(f"✅ API сервер запущен на порту {WEB_PORT}")
