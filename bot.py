...
def main_keyboard(user_id):
    buttons = [
        ...
        [KeyboardButton(text="🎁 Ежедневный подарок")],
...
async def daily_gift(message: Message):
    ROULETTE_URL = "https://ВАШ_GITHUB.github.io/roulette.html"  # замени на свой URL
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text="🎰 Открыть рулетку",
            web_app={"url": ROULETTE_URL}
        )
    ]])
    await message.answer(
        "🎁 <b>Ежедневный подарок</b>\n\n"
        "Крути рулетку — шанс выиграть подписку на 1 или 3 дня!\n"
        "Одна прокрутка в сутки.",
        reply_markup=kb,
        parse_mode="HTML"
    )
...
async def subscription_checker():
    while True:
        for row in db_users_expired():
            ...
            # Check if already notified (stored in DB)
            already = db_setting_get(f"notified_{uid}_{key_used}")
            if already:
                continue
            try:
                ...
                # Mark notified in DB so survives restarts
                db_setting_set(f"notified_{uid}_{key_used}", "1")
                # DELETE from users so subscription is disabled
                with get_conn() as conn:
