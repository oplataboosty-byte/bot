...
def init_db():
    with get_conn() as conn:
        ...
        # привязка HWID устройства — один ключ = одно устройство
        if "hwid" not in cols2:
            conn.execute("ALTER TABLE users ADD COLUMN hwid TEXT NOT NULL DEFAULT ''")
...
def db_user_find_by_key(key):
    ...
    if not row:
        return None
    cols = [d[0] for d in conn.execute("PRAGMA table_info(users)").fetchall()] if False else []
    return {"user_id": row["user_id"], "expires": datetime.fromisoformat(row["expires"]),
            "key": row["key"], "username": row["username"],
            "hwid": row["hwid"] if "hwid" in row.keys() else ""}

def db_user_bind_hwid(user_id, hwid):
    with get_conn() as conn:
        conn.execute("UPDATE users SET hwid=? WHERE user_id=?", (hwid, user_id))
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    welcome_text = (
        ...
        f"🕐 Поддержка работает с <b>9:00 до 20:00</b> (по МСК)."
    )
...
async def subscription_checker():
    notified = set()
    while True:
        for row in db_users_expired():
            uid = row["user_id"]
            if uid not in notified:
                try:
                    key_used = row.get("key", "")
                    uname_used = row.get("username", "")
                    key_text = f"<code>{key_used}</code>" if key_used else "ваш ключ"
                    kb_buy = InlineKeyboardMarkup(inline_keyboard=[[
                        InlineKeyboardButton(text="💳 Купить ключ", url=PAYMENT_URL)
                    ]])
                    await bot.send_message(
                        uid,
                        f"⏰ Ключ {key_text} закончил своё действие!\n\n"
                        f"Нажмите <b>Купить Ключ</b> чтобы получить новый! 👇",
                        reply_markup=kb_buy,
                        parse_mode="HTML"
                    )
                    notified.add(uid)
                    db_log("sub_expired_notified", uid, uname_used, f"key={key_used}")
                except Exception:
                    pass
...
# ── ВЕБ-СЕРВЕР (проверка ключей из Java-меню) ─────────────────────────────────
# Java шлёт: GET /check?key=Yrener1234&hwid=abc123
# Ответы:    OK | NOT_FOUND | EXPIRED | WRONG_HWID
async def handle_check(request: web.Request) -> web.Response:
    key  = request.rel_url.query.get("key",  "").strip()
    hwid = request.rel_url.query.get("hwid", "").strip()
    if not key or not hwid:
        return web.Response(text="BAD_REQUEST")

    user = db_user_find_by_key(key)
    if not user:
        db_log("menu_check", details=f"NOT_FOUND key={key}")
        return web.Response(text="NOT_FOUND")

    if datetime.now() > user["expires"]:
        db_log("menu_check", user_id=user["user_id"], details=f"EXPIRED key={key}")
        return web.Response(text="EXPIRED")

    stored_hwid = user.get("hwid", "")
    if not stored_hwid:
        # первый запуск — привязываем HWID
        db_user_bind_hwid(user["user_id"], hwid)
        db_log("menu_check", user_id=user["user_id"], username=user["username"],
               details=f"OK+BIND key={key} hwid={hwid}")
        return web.Response(text="OK")

    if stored_hwid != hwid:
        db_log("menu_check", user_id=user["user_id"], username=user["username"],
               details=f"WRONG_HWID key={key}")
        return web.Response(text="WRONG_HWID")

    db_log("menu_check", user_id=user["user_id"], username=user["username"],
           details=f"OK key={key}")
    return web.Response(text="OK")
...
async def main():
    ...
    # Веб-сервер для Railway (Railway требует слушать PORT)
    app = web.Application()
