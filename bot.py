...
async def handle_check_key(request):
    ...
    if secret != API_SECRET:
        return aiohttp_web.Response(
            text=_json.dumps({"valid": False, "reason": "forbidden"}),
            content_type="application/json", status=403
        )
    if not key:
        return aiohttp_web.Response(
            text=_json.dumps({"valid": False, "reason": "no_key"}),
            content_type="application/json"
        )
    ...
    if not row:
        return aiohttp_web.Response(
            text=_json.dumps({"valid": False, "reason": "not_found"}),
            content_type="application/json"
        )
    ...
    if now > expires:
        return aiohttp_web.Response(
            text=_json.dumps({"valid": False, "reason": "expired"}),
            content_type="application/json"
        )
...
async def start_web_server():
    app_web = aiohttp_web.Application()
    app_web.router.add_get("/health",    handle_health)
    app_web.router.add_get("/check_key", handle_check_key)
    runner = aiohttp_web.AppRunner(app_web)
    await runner.setup()
